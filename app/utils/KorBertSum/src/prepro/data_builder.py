...
def combination_selection(doc_sent_list, abstract, summary_size):
    def _rouge_clean(s):
...
def format_to_bert(args):
    ...
    for corpus_type in datasets:
        ...
        print('------------------------------------------------')
        print(glob.glob(pjoin(args.raw_path, '*' + corpus_type + '.jsonl')))
        print('------------------------------------------------')
...
def _format_to_bert(params):
    ...
    datasets = []
    for d in tqdm(jobs):
        source, tgt = d['article_original'], d['abstractive'][0]
...
def format_to_lines(args):
    corpus_mapping = {}
    for corpus_type in ['valid', 'train']:
        temp = []
    ...
    for corpus_type in ['train', 'valid']:
